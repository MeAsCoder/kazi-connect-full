...
export async function matchWorkers(opts: {
  ...
  // Multi-label: the model may return several trades (e.g. electrician + plumber for
  // "fix my appliances and my shower head"). Rank workers across all matched trades.
  const tradeList = prediction.trades?.length ? prediction.trades : [prediction.trade];

  const workers = await prisma.workerProfile.findMany({
    where: { trade: { in: tradeList }, isAvailable: true, user: { isBanned: false } },
  ...
  const ranked = workers
    .map((w) => {
      ...
      // Primary predicted trade scores full skill; co-trades (multi-label) slightly less.
      const skill = w.trade === prediction.trade ? 1.0 : 0.9;
